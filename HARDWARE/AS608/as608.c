...
{   
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //PB4设置成输入，默认下拉  
...
void Add_FR(void)
{
  ...
  while(1)
  {
    switch (processnum)
    {
			case 0:
      i++;
      OLED_ShowString(20,3,"Press FR",16);
			while(!PS_Sta) 
			{
      ...
      if(ensure == 0x00)
      {
        ...
        else printf("%s",(u8*)EnsureMessage(ensure));
      }
      else printf("%s",(u8*)EnsureMessage(ensure));
      break;
    ...
    case 1:
      i++;
      OLED_ShowString(0,3,"Press FR again",16);
			while(!PS_Sta) {}
      ensure = PS_GetImage();
      if(ensure == 0x00)
      {
    ...
    case 2:
			OLED_ShowString(10,3,"Wait a moment",16);
			//printf("对比指纹\n");
      ensure = PS_Match();
      if(ensure == 0x00)
      {
    ...
    case 4:
			//printf("按B8加,按A1减,按B1保存");
			OLED_ShowString(0,0,"K5 ID+",16);
			OLED_ShowString(0,2,"K2 ID-",16);
			OLED_ShowString(0,4,"K3 ID Save",16);
			OLED_ShowString(0,6,"ID = ",16);
      while(key_num != 3)
      {
        key_num = KEY_Scan(0);
				//printf("KEY1=%d,KEY2=%d,KEY3=%d,KEY4=%d,KEY5=%d,key_num=%d\n",(u8)KEY1,(u8)KEY2,(u8)KEY3,(u8)KEY4,(u8)KEY5,key_num);
        if(key_num == 2)
        {
          key_num = 0;
          if(ID_NUM > 0)
      ...
      if(ensure == 0x00)
      {
				OLED_ShowString(0,3,"FR Save Success",16);
				//printf(" 录入指纹成功");
        delay_ms(2000);
				OLED_Clear();
				delay_ms(10);
...
void Del_FR(void)
{
  ...
  u16 ID_NUM = 0;
	u8 key_num;
	//printf("按B8加,按A1减,按B1保存,A0清空指纹库\n");
	OLED_ShowString(0,0,"K5 ID+",16);
	OLED_ShowString(0,2,"K2 ID-",16);
	OLED_ShowString(0,4,"K3 ID Save",16);
	OLED_ShowString(0,6,"ID = ",16);
...
//    }
  ...
  if(ensure == 0)
  {
		OLED_ShowString(0,3,"Delete Success",16);
		//printf("删除指纹成功\n");
  }
  else
    printf("%s",(u8*)EnsureMessage(ensure));
